...
class PlayAudioActivity : AppCompatActivity() {
    ...
    fun prepare(): Boolean {
        try {
            ...
            Log.e("노래99", mList!![0])
            Log.e("노래99", e.localizedMessage)
            return false
        } catch (e: IOException) {
            Log.e("노래88", mList!![0])
            Log.e("노래88", e.localizedMessage)
            return false
        }
        Log.e("노래88", mList!![100])
        return true
    }
    ...
    // 재생 위치 이동
    var mOnSeek: OnSeekBarChangeListener = object : OnSeekBarChangeListener {
    ...
    // 재생 완료되면 다음곡으로
    var mOnComplete = OnCompletionListener {
    ...
    // 위치 이동 완료 처리
    var mOnSeekComplete = OnSeekCompleteListener {
        if (wasPlaying) {
            mPlayer!!.start()
        }
    }
