    ...
    var clickHandler: View.OnClickListener = object : View.OnClickListener{
        override fun onClick(v: View) {
            if (v === btn_contacts) {
                ...
                if (ContextCompat.checkSelfPermission(this@BasicAppActivity, Manifest.permission.CALL_PHONE) === PackageManager.PERMISSION_GRANTED) {
                    val intent = Intent(Intent.ACTION_CALL, Uri.parse("tel:010-3790-1997"))
                    startActivity(intent)
                } else {
                    ActivityCompat.requestPermissions(this@BasicAppActivity, arrayOf<String>(Manifest.permission.CALL_PHONE), 100)
                }
            }
        }
    }
